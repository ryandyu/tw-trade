...
@Component
public class IoiRequestor implements Processor
{

    private static final Logger LOG = LoggerFactory.getLogger(IoiRequestor.class);
...
//            }
        ...
        List<IOI> iois = new LinkedList<IOI>();
        
//        if (!quoteSets.isEmpty())   {
//        	iois.add(new IOIBuilder().quoteId(UUID.randomUUID().toString()).quoteSets(quoteSets).build(new Side(Side.BUY)));
//           	iois.add(new IOIBuilder().quoteId(UUID.randomUUID().toString()).quoteSets(quoteSets).build(new Side(Side.SELL)));
//
//            if (!updateQuoteSets.isEmpty())   {
//                iois.add(new IOIBuilder().quoteId(UUID.randomUUID().toString()).quoteSets(updateQuoteSets).build(new Side(Side.BUY)));
//               	iois.add(new IOIBuilder().quoteId(UUID.randomUUID().toString()).quoteSets(updateQuoteSets).build(new Side(Side.SELL)));
//            }
//        }

        exchange.getIn().setBody(iois);
    }
}
